...
- SQLite (`./data/app.db`) is the single source of truth for all reads and writes — the config invariants above (transactional ID assignment, undeletable references) all depend on that. `config/*.csv` is a **generated snapshot**, never a second live read path.
- `lib/config-sync.ts`'s `syncConfigSnapshot()` re-exports the full current `mandanten`/`sites`/`mediums`/`campaigns`/`tids` tables (plus each medium's site restriction, nested as a comma-joined `siteIds` cell, e.g. `103,104`) to `config/mandanten.csv`, `config/sites.csv`, `config/mediums.csv`, `config/campaigns.csv`, `config/tids.csv` using the same semicolon-separated, UTF-8-BOM `unparseCsv` helper as every other export in the app — deliberately so it's easy to open and hand-edit in (German) Excel. It's called from `revalidateAdmin()` in `app/actions/admin.ts` (every admin mutation, including `deleteTid`) and directly from `generateUrl`/`bulkGenerate` after a tid is inserted. It's best-effort — a write failure is logged, never surfaced as an action error, since the DB write already succeeded and is authoritative.
- `lib/db/init.ts`'s seed step reads these same files (`readConfigSeedRows`, via `parseCsv`) when bootstrapping an **empty** table: if `config/<table>.csv` exists and is non-empty, its rows (including their IDs) become the seed data instead of the small hardcoded defaults. This only ever runs once per table (guarded by `SELECT COUNT(*)`), so editing `config/` after first boot does nothing on its own — it only matters for a fresh `./data/app.db`. Seeding order matters for FK integrity: mandanten → sites → mediums (+ site_mediums) → campaigns. `tids.csv` is the one exception: it's written for audit/review only and is never read back — a fresh DB always starts with zero generated tracking-IDs, regardless of what `config/tids.csv` contains.
- CSV exports triggered from the admin tabs themselves remain separate, user-initiated downloads (`unparseCsv`/`downloadTextFile`) — unrelated to `config/`, which is never downloaded manually and is meant to be committed/inspected/hand-edited as a repo file directly.
...
- `config/` — generated CSV snapshot of master data + initial seed source, see "Config folder" above
- `lib/db/` — Drizzle schema, migrations, seed
- `lib/config-sync.ts` — writes the `config/*.csv` snapshot after admin mutations and tid generation/deletion
...
After changes touching campaigns, Mandanten, or the config folder, also verify: create a Mandant, create a campaign selecting it, confirm the composed display (`<campid> / <Mandant> - <name> - MM/YYYY`) shows up correctly in the Generator dropdown and the Verlauf table, and confirm `config/campaigns.csv`/`config/mandanten.csv` (etc.) were rewritten to reflect the change.
...
After changes touching tid creation or deletion, also verify: generating a URL updates `config/tids.csv`; as an admin, deleting a tid from the Verlauf table removes it from both the table and `config/tids.csv`; and the "Löschen" column/button is absent entirely when logged in as `user`.
