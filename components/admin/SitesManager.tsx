...
import { createSite, deleteSite, updateSite } from "@/app/actions/admin";
...
type Site = { siteid: number; name: string; createdAt: string };
...
export function SitesManager({ rows }: { rows: Site[] }) {
  ...
  async function saveEdit(siteid: number) {
    ...
    setRowError(null);
    try {
      const res = await updateSite(siteid, editName);
      if (!res.ok) {
  ...
  function handleExport() {
    const csv = unparseCsv(
      ["siteid", "name", "createdAt"],
      rows.map((r) => [r.siteid, r.name, r.createdAt]),
    );
